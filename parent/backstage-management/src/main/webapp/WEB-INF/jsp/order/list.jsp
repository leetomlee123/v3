...
<HEAD>
    ...
    <script type="text/javascript">
        ...
        function findOrderInfoByOid(oid) {
            ...
            //ajax异步访问数据
            $.post(
                "${pageContext.request.contextPath }/order/orderitem",
                {"oid": oid},
                function (data) {
                    ...
                    for (var i = 0; i < data.products.length; i++) {
                        content += "<tr style='text-align: center;'>" +
                            "<td>" +
                            "<img src='${pageContext.request.contextPath }/" + data.products[i].pimage + "' width='70' height='60'>" +
                            "</td>" +
                            "<td><a target='_blank'>" + data.products[i].pname + "</a></td>" +
                            "<td>￥" + data.products[i].shop_price + "</td>" +
                            "<td>" + data.orderItems[i].count + "</td>" +
                            "<td><span class='subtotal'>￥" + data.orderItems[i].subtotal + "</span></td>" +
                            "</tr>";
                    }
        ...
        <tr>
            <td class="ta_01" align="center" bgColor="#f5fafe">
                <table cellspacing="0" cellpadding="1" rules="all"
                    ...
                    <c:forEach items="${pageBean.content }" var="order" varStatus="vs">
        ...
        <tr align="center">
            <td colspan="7">
                ...
                <c:if test="${pageBean.hasPrevious()==true}">
                    <a href="${ pageContext.request.contextPath }/order/0">首页</a>|
                    <a href="${ pageContext.request.contextPath }/order/${pageBean.previousPageable().pageNumber}">上一页</a>|
                </c:if>
                <c:if test="${pageBean.hasNext()==true}">
                    <a href="${ pageContext.request.contextPath }/order/${pageBean.nextPageable().pageNumber}">下一页</a>|
                    <a href="${ pageContext.request.contextPath }/order/${pageBean.totalPages-1}">尾页</a>|
                </c:if>
            </td>
        </tr>
        </TBODY>
...
<div id="showDiv" class="blk" style="display:none;">
    <div class="main">
        ...
        <div id="loading" style="padding-top:30px;text-align: center;">
            <img alt="" style="width:100px;height:100px;" src="${pageContext.request.contextPath }/order/loading.gif">
        </div>
